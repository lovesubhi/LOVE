...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("Love/assets/bg2.jpg")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((450, 450))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('Love/assets/font.ttf', size=40)
    welcome_font = ImageFont.truetype('Love/assets/font.ttf', size=60)
...
➖➖➖➖➖➖➖➖➖➖➖➖**
""",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"⦿ ᴀᴅᴅ ᴍᴇ ⦿", url=f"https://t.me/LoveBot?startgroup=true")]])
        )
    except Exception as e:
        LOGGER.error(e)
    try:
